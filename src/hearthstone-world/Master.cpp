...
#define BANNER "WowEmu r%u/%s-%s-%s :: World Server\n"
...
#ifdef WIN32
        static const char* default_config_file = "configs/wowemu-world.conf";
        static const char* default_realm_config_file = "configs/wowemu-realms.conf";
#else
        static const char* default_config_file = CONFDIR "configs/wowemu-world.conf";
        static const char* default_realm_config_file = CONFDIR "configs/wowemu-realms.conf";
#endif
...
bool Master::Run(int argc, char ** argv)
{
	...
	if(geteuid() == 0 || getegid() == 0)
		Log.LargeErrorMessage( LARGERRORMESSAGE_WARNING, "You are running WowEmu as root.", "This is not needed, and may be a possible security risk.", "It is advised to hit CTRL+C now and", "start as a non-privileged user.", NULL);
#endif
	...
	if( Config.MainConfig.SetSource( config_file ) )
		Log.Success( "Config", ">> configs/wowemu-world.conf" );
	else
	{
		Log.Error( "Config", ">> configs/wowemu-world.conf" );
		return false;
	}
	...
	if(Config.RealmConfig.SetSource(realm_config_file))
		Log.Success( "Config", ">> configs/wowemu-realms.conf" );
	else
	{
		Log.Error( "Config", ">> configs/wowemu-realms.conf" );
		return false;
	}
